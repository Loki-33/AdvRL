...
env = BattleBotEnv(render_mode='rgb_array')
    ...
    for ep in range(episodes):
        ...
        for step in range(n_steps):
            ...
            with torch.no_grad():
                ...
                #actions 
                action_bot1 = dist.sample()
                action_bot2 = dist.sample()

                log_prob1 = dist.log_prob(action_bot1)
                log_prob2 = dist.log_prob(action_bot2)
                log_prob = log_prob1+log_prob2 

                entropy = dist.entropy()

            next_state, reward, done, truncated, _ = env.step([action_bot1.item(), action_bot2.item()])
            
            states.append(state)
            actions.append([action_bot1.item(), action_bot2.item()])
            ...
            actions_bot1.append(action_bot1.item())
            state = next_state
        ...
        actions_array = np.array(actions_bot1)
        action_counts = np.bincount(actions_array, minlength=5)
        ...
        for epoch in range(n_epochs):
            ...
            for start in range(0, dataset_size, mini_batchsize):
                ...
                new_log_prob1 = dist.log_prob(mb_actions[:, 0])  # Bot 1 actions
                new_log_prob2 = dist.log_prob(mb_actions[:, 1])  # Bot 2 actions
                new_log_probs = new_log_prob1 + new_log_prob2
