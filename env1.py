...
# this env has fixed reward mechanism so that the agent doesnt exploit the loophole
class CleanEnv2(gym.Env):
    metadata = {'render_mode':['human', 'rgb_array'], 'render_fps':30}
    ...
    def __init__(self, width=800, height=600, render_mode='rgb_array', max_steps=200):
        ...
        self.natural_messes = set()
        self.created_messes = set()
        self.messes = set()
        self.current_step=0
        # agent 
        self.agent_x = 0
    ...
    def reset(self, seed=None, options=None):
        ...
        grid_cells = [(c,r) for c in range(cols) for r in range(rows) if not (c==center_col and r==center_row)]
        self.natural_messes = set(random.sample(grid_cells, min(10, len(grid_cells))))
        self.created_messes = set()
        self.messes = self.natural_messes.copy()
    ...
    def step(self, action):
        ...
        if action == 0:  # UP
            ...
            agent_col = self.agent_x // self.grid_width
            agent_row = self.agent_y // self.grid_height
            pos = (agent_col, agent_row)
            if pos in self.messes:
                self.messes.remove(pos)
                # Only reward cleaning NATURAL messes
                if pos in self.natural_messes:
                    reward = 1.0
                    self.natural_messes.remove(pos)
                    self.score += 1
                elif pos in self.created_messes:
                    reward = 0.0  # No reward for cleaning own mess
                    self.created_messes.remove(pos)
            else:
                reward = -0.1
        elif action == 5:  # MAKE_MESS
            agent_col = self.agent_x // self.grid_width
            ...
            if pos not in self.messes:
                self.messes.add(pos)
                self.created_messes.add(pos)
                reward = -0.5
            else:
                reward = -0.1
...
#    env = CleanEnv2(render_mode='human')
#    obs, info = env.reset()
#    
#    running = True
#    done=False
